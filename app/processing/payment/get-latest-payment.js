...
const getLatestPayment = (paymentRequest, settlement, supportingSettlements) => {
  ...
  const latestPayment = {
    ...
    period: paymentRequest.schedule
      ? getPaymentSchedule(paymentRequest.schedule, paymentRequest.dueDate, settlement.paymentValue, settlement.value, settlement.lastSettlementValue, paymentRequest.value, settlement.settlementDate)
