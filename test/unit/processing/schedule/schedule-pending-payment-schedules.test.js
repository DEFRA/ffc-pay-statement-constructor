...
jest.mock('../../../../app/processing/schedule/get-scheduled-payment-schedules')
const getScheduledPaymentSchedules = require('../../../../app/processing/schedule/get-scheduled-payment-schedules')
...
describe('batch schedule', () => {
  ...
  test('should call getScheduledSettlements', async () => {
    ...
    expect(getScheduledPaymentSchedules).toHaveBeenCalled()
  })

  test('should call getScheduledSettlements once', async () => {
    await schedulePendingPaymentSchedules()
    expect(getScheduledPaymentSchedules).toHaveBeenCalledTimes(1)
  })

  test('should call getScheduledSettlements with new Date() and mockTransaction', async () => {
    await schedulePendingPaymentSchedules()
    expect(getScheduledPaymentSchedules).toHaveBeenCalledWith(new Date(), mockTransaction)
  })
  ...
  test('should not throw when getScheduledSettlements throws', async () => {
    getScheduledPaymentSchedules.mockRejectedValue(new Error('Database retrieval issue'))
  ...
  test('should call mockTransaction.rollback when getScheduledSettlements throws', async () => {
    getScheduledPaymentSchedules.mockRejectedValue(new Error('Database retrieval issue'))
    try { await schedulePendingPaymentSchedules() } catch { }
