...
describe('get statement', () => {
  ...
  describe('when all information is present', () => {
    ...
    test('returns annualValue as statement.funding.annualValue', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(result.funding[0].annualValue).toEqual(statement.funding[0].annualValue)
    })
    ...
    test('returns area as string', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(result.funding[0].area).toEqual(expect.any(String))
    })

    test('returns area as statement.funding.area', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(result.funding[0].area).toEqual(statement.funding[0].area)
    })

    test('returns quarterlyPayment key within funding key', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(Object.keys(result.funding[0])).toContain('quarterlyPayment')
    })

    test('returns quarterlyPayment as string', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(result.funding[0].quarterlyPayment).toEqual(expect.any(String))
    })

    test('returns quarterlyPayment as statement.funding.quarterlyPayment', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(result.funding[0].quarterlyPayment).toEqual(statement.funding[0].quarterlyPayment)
    })

    test('returns quarterlyReduction key within funding key', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(Object.keys(result.funding[0])).toContain('quarterlyReduction')
    })

    test('returns quarterlyReduction as string', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(result.funding[0].quarterlyReduction).toEqual(expect.any(String))
    })

    test('returns quarterlyReduction as statement.funding.quarterlyReduction', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(result.funding[0].quarterlyReduction).toEqual(statement.funding[0].quarterlyReduction)
    })

    test('returns quarterlyValue key within funding key', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(Object.keys(result.funding[0])).toContain('quarterlyValue')
    })

    test('returns quarterlyValue as string', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(result.funding[0].quarterlyValue).toEqual(expect.any(String))
    })

    test('returns quarterlyValue as statement.funding.quarterlyValue', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(result.funding[0].quarterlyValue).toEqual(statement.funding[0].quarterlyValue)
    })

    test('returns rate key within funding key', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(Object.keys(result.funding[0])).toContain('rate')
    })

    test('returns rate as string', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(result.funding[0].rate).toEqual(expect.any(String))
    })

    test('returns rate as statement.funding.rate', async () => {
      const result = await getStatement(schedule.settlementId, schedule.scheduleId)
      expect(result.funding[0].rate).toEqual(statement.funding[0].rate)
    })
